...
def tiler(imnames, newpath,newlabels, falsepath, slice_size, ext, label_path):
    for imname in imnames:
        im = Image.open(imname)
        imr = np.array(im, dtype=np.uint8)
        height = imr.shape[0]
        width = imr.shape[1]
        labname = label_path+ '/'+imname.strip().split("/")[-1].split(".")[0]+".txt"
        ...
        for i in range((height // slice_size)):
            for j in range((width // slice_size)):
                ...
                for box in boxes:
                    if pol.intersects(box[1]):
                        ...
                        if not imsaved:
                            ...
                            slice_labels_path = newlabels + "/" + filename.replace(ext, f'_{i}_{j}.txt')                            
                            print(slice_path)
                ...
                if len(slice_labels) > -1:
                   ...
                   # slice_path = newpath + "/" + filename.replace(ext, f'_{i}_{j}{ext}')                            
                    slice_labels_path = newlabels + "/" + filename.replace(ext, f'_{i}_{j}.txt')      
                    slice_df = pd.DataFrame(slice_labels_path , columns=['class', 'x1', 'y1', 'w', 'h'])
                    print(slice_df)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-labels", default="./yolosample/ts/", help = "Source folder with labels")
    parser.add_argument("-target", default="./yolosliced/ts/", help = "Target folder for a new sliced dataset")
    parser.add_argument("-labeltarget", default="./yolosliced/ts/", help = "Target folder for a new sliced dataset")
    parser.add_argument("-ext", default=".png", help = "Image extension in a dataset. Default: .JPG")
    ...
    labnames = glob.glob(f'{args.labels}/*.txt')
    ...
    tiler(imnames, args.target,args.labeltarget, args.target, args.size, args.ext, args.labels)
